...
def sum_until_prime_input():
    ...
    x=3
    while x >2:
        x=int(input('Integer greater than 2: '))
        sum=sum+x
        if is_prime(sum)==True:
            break
    print('The sum of the input integers is '+str(sum))
...
def run_test_wait_for_sum_of_cubes():
    ...
    print('TEST STARTED!  Has it ended?')
    expected = 4
    actual = wait_for_sum_of_cubes(37
                                   )
    print('Expected:', expected)
    print('Actual:  ', actual)
    print('TEST ENDED!')
